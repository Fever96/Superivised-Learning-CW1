...
#Author:Yihang OU
#Version 1.0
#Title: Question 3 for CW1 for Supervised Learning 18/19 in UCL
...
#use input data x and the dimension of sin(n*pi*x) bases k
def feature_map2(x,k):
    X=[]
    for i in range(k+1):
        X.append(np.sin((i+1)*np.pi*x))
    X=np.mat(X).T
...
def fit(x,y,k):
    X=np.around(feature_map2(x,k).astype('float64'),decimals = 7) #feature map
